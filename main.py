...
async def register_info(message: types.Message):
    ...
    async with db.begin() as session:
        ...
        if not user:
            await message.answer('Введите код-приглашение в формате:\n'
...
async def register(message: types.Message):
    ...
    try:
        ...
        await message.answer("Введите код в корректном формате:\n"
                             "Код: 10-ти значный код")
        return

    async with db.begin() as session:
        hashed_admin_pass_query = select(Admin.admin_code)
        hashed_admin_pass_result = (await session.execute(hashed_admin_pass_query)).fetchone()
            ...
            if code:
                ...
                if code_result:
                    code_update = update(
                        ...
                        Code.code == code, Code.is_used == False
                    ).values(is_used=True, use_time=datetime.datetime.utcnow())
                    await session.execute(code_update)
            else:
                await message.answer("Убедитесь, что вы ввели корректный код-приглашение.")
                return
            user_info = {
                'user_id': user_id,
...
async def get_codes(message: types.Message):
    ...
    if user_codes:
        await message.answer(
            '{}\n{}'.format('\n'.join([f'{"-" * 21}\n`{code.code}`' for code in user_codes]), "-" * 21),
            parse_mode='Markdown'
        )
        return
...
async def add_chunk_codes(message: types.Message):
    ...
    async with db.begin() as session:
        user_query = select(User).options(
            selectinload(
                User.code)).join(Code).where(
            Code.user_relationship == user_id)
...
async def error_handler(message: types.Message):
    user_id = message.from_user['id']
    logger.info(f'User {user_id}')
    await message.answer("Введите код в корректном формате, если вы хотите зарегистрироваться\n\n"
                         "Подробнее: /register")
